# Especificación de Arquitectura de Frontend - Furry Feast MVP (v2)
...
**Objetivo:** Refinar la arquitectura de frontend alineándola con el PRD y el análisis de mockups para la Fase 3.

---

## Cambios aplicados en esta revisión (v2)

*   **Ruta `/registro` clasificada como Opcional:** Se ha degradado la prioridad de la vista de registro público del cliente. Se marca explícitamente como **funcionalidad opcional / futura** (fuera del alcance del MVP actual). El inicio de sesión se gestiona con credenciales pre-cargadas para los roles `ROLE_ADMIN` y `ROLE_CUSTOMER`.
*   **Consolidación de Catálogo Unificado:** Se eliminó cualquier ambigüedad sobre vistas separadas para dispositivos móviles y de escritorio. El catálogo de clientes se implementará exclusivamente en la ruta `/` utilizando el componente único `CatalogoClientes`, el cual será responsivo mediante CSS moderno (Flexbox, Grid y Media Queries).
*   **Priorización de Alertas en Dashboard:** Se rediseñó la descripción de la pantalla principal del administrador (`/admin/dashboard`). Ahora se define que el componente de alertas de stock bajo (`stock_actual <= stock_minimo`) es el **elemento prioritario y central** de la vista (según RF-3.3 y el Caso de Uso 2 del PRD), y no un widget de KPIs secundarios.
*   **Preservación de Core Técnico:** Se mantuvieron intactos la estructura de carpetas, el sistema de rutas dinámicas de React Router DOM v6, los layouts (`PublicLayout` y `AdminLayout`), el componente `<ProtectedRoute />`, los contextos de autenticación y transacciones, y el desacoplamiento de servicios HTTP asíncronos con soporte de mock data local.
...
│   │   ├── common/          # Componentes transversales: Buttons, Inputs, Modals, Loaders, ProtectedRoute
...
│   │   ├── public/          # Catálogo de Clientes, Login, Acceso Denegado
...
│   │   ├── authService.js   # Peticiones de login
...
El ruteo se gestiona de forma centralizada en `App.jsx` utilizando `react-router-dom` (versión 6). Separamos la navegación en rutas públicas abiertas y rutas protegidas exclusivas de administración.

```mermaid
graph TD
    A[Inicio de la App] --> B{¿Ruta Solicitada?}
    B -->|Públicas| C[Navbar Pública]
    C --> C1[/: Catálogo Público Único y Responsivo]
    C --> C2[/login: Login]
    C --> C3[// Opcional: /registro - Futuro]
    
    B -->|Privadas Admin| D[Layout Administrativo]
    D -->|Filtro Rol === ADMIN| D1[/admin/dashboard: Alertas de Stock + KPIs]
...
| :--- | :--- | :--- | :--- | :--- |
| `/` | `CatalogoClientes` | `PublicLayout` | Cualquiera (Público) | Consulta interactiva, paginada y filtrable de productos. Componente responsivo unificado para móvil y escritorio. |
| `/login` | `Login` | `PublicLayout` | Cualquiera (No logueado) | Autenticación y obtención del token JWT. |
| `/registro` | *RegistroCliente (Opcional)* | `PublicLayout` | Cualquiera (No logueado) | **[OPCIONAL / FUERA DE MVP]** Formulario de registro para nuevos clientes finales. |
| `/denegado` | `Forbidden` | `PublicLayout` | Cualquiera | Mensaje de advertencia HTTP 403 (Acceso Denegado). |
| `/admin/dashboard` | `DashboardAdmin` | `AdminLayout` | `ROLE_ADMIN` | Pantalla operativa central con módulo prioritario de Alertas de Stock Bajo y KPIs. |
...
*   **Contenido Dinámico (`<Outlet />`):** Renderiza el catálogo o el login.
...
*   **Barra Superior (Topbar):** Muestra el perfil del administrador autenticado, alertas críticas rápidas de stock bajo, y un botón para volver rápidamente al catálogo público.
...
## 4. Descripción de Páginas o Vistas Principales

### A. Catálogo Público (`CatalogoClientes.jsx`)
Vista única optimizada mediante CSS responsive para visualizar el listado de productos de forma adaptativa.
*   **Buscador y Filtro:** Barra de texto y dropdown de categorías (Alimentos, Medicamentos, Accesorios, Cuidado) integrados en el área superior.
*   **Grilla Responsiva:** Tarjetas de productos que se reordenan dinámicamente de 1 columna (móvil) a 4 columnas (escritorio).
*   **Paginación:** Barra inferior con números de página y botones Anterior/Siguiente.

### B. Dashboard Administrativo (`DashboardAdmin.jsx`)
Es la pantalla de inicio operativa del Administrador tras autenticarse.
*   **Panel de Alertas de Stock Bajo (Eje Central de la Vista):** Ubicado de manera prominente en la sección superior o central de la pantalla. Consume el endpoint asíncrono `/api/productos/alertas` para mostrar una lista urgente de productos donde `stock_actual <= stock_minimo` con colores e indicadores llamativos de advertencia para que el administrador reabastezca de inmediato.
*   **Sección de KPIs Rápidos:** Contenedores numéricos que detallan el total de productos en catálogo, número de productos agotados y total de ventas registradas hoy.

---

## 5. Estrategia de Gestión de Estado Global (Context API)
...
## 6. Protección de Rutas (Guardias de Navegación)
...
## 7. Preparación para API REST y Estrategia de Mock Data
